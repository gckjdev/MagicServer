...
public class MongoDBClient {
    ...
    public MongoDBClient(String serverAddress, String dbName, String userName,
        ...
        try {
            this.mongo = new MongoClient(serverAddress, 27017);
    ...
    public MongoDBClient(String dbName) {
        ...
        try {
            ...
            builder.maxWaitTime(30 * 1000);        // 30 seconds for thread block wait
//			mongoOptions.connectTimeout = 30*1000;
//			mongoOptions.socketTimeout = 60*1000;

            MongoClientOptions mongoOptions = builder.build();
            this.mongo = new MongoClient(serverAddress, mongoOptions);
            log
                    .info("<MongoDBClient> mongo option="
                            + this.mongo.getMongoClientOptions().toString()
                            + ", connectionPerHost="
                            + this.mongo.getMongoClientOptions().getConnectionsPerHost()
                            + ", threadsAllowedToBlockForConnectionMultiplier="
                            + this.mongo.getMongoClientOptions().getThreadsAllowedToBlockForConnectionMultiplier());
